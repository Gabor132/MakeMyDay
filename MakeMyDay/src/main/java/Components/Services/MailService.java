...
public class MailService {
        ...
        SESSION = Session.getInstance(PROPS,
            new Authenticator() {
                ...
                protected PasswordAuthentication getPasswordAuthentication() {
                        return new PasswordAuthentication(PROPS.getProperty("server_email"), PROPS.getProperty("server_password"));
                }
            }
        );
    }
    ...
    public boolean sendNotifications(List<Plan> plans, MailType type){
        ...
        for(Plan plan : plans){
            try {
                ...
                Logger.getLogger(MailService.class.getName()).log(Level.INFO, "Preparing email for {0}", plan.getUser().getEmail());
                Message message = new MimeMessage(SESSION);
                message.setFrom(new InternetAddress(PROPS.getProperty("server_email")));
                message.setRecipients(Message.RecipientType.TO,
                        InternetAddress.parse(plan.getUser().getEmail()));
    ...
    public boolean sendConfirmation(User user){
        try {
            ...
            String linkConfirmare = "<a href=\"" + PROPS.getProperty("server_url")+"?link=" + link + 
                    "\">link</a>";
