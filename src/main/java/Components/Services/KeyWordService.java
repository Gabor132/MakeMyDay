    ...
    public void updatePercentages() {
        ...
        for (EventTypeWord etw : eventTypeWords) {
            ...
            Logger.getLogger(KeyWordService.class.getTypeName()).log(Logger.Level.INFO,
                    "evenimente de tip(" + etw.getType().getType() + "): " + ET + "\n" + "evenimente ce contin("
                            + etw.getWord().getWord() + "): " + EK + "\n" + "intersectia: " + ETK);
            double newPercent = 0;
            if (ETK != 0.0 && ET != 0.0) {
                newPercent = ((double) ETK / (double) ET);
            }
            etw.setPercentage(newPercent);
            Logger.getLogger(KeyWordService.class.getTypeName()).log(Logger.Level.INFO, etw.getType().getType() + " - "
                    + etw.getWord().getWord() + " changed from " + percent + " to " + newPercent);
